...
func sizeOfHashObject(obj *model2.HashObject) int {
	if obj.GetEncoding() == model2.ZipListEncoding {
		extra := obj.Extra.(*model2.ZiplistDetail)
...
func sizeOfSetObject(obj *model2.SetObject) int {
	if obj.GetEncoding() == model2.IntSetEncoding {
		extra := obj.Extra.(*model2.IntsetDetail)
		return extra.RawStringSize
	}
