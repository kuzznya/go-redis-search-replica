...
func sizeOfListObject(obj *model2.ListObject) int {
	switch obj.GetEncoding() {
	case model2.QuickListEncoding:
		detail := obj.Extra.(*model2.QuicklistDetail)
		return sizeOfQuicklist(detail)
	case model2.ListEncoding:
		return sizeOfList(obj.Values)
	case model2.ZipListEncoding:
		return sizeOfZiplist(obj.Values)
	case model2.QuickList2Encoding:
		detail := obj.Extra.(*model2.Quicklist2Detail)
...
func sizeOfQuicklist(detail *model2.QuicklistDetail) int {
...
func sizeOfQuicklist2(values [][]byte, detail *model2.Quicklist2Detail) int {
	...
	for i, enc := range detail.NodeEncodings {
		if enc == model2.QuicklistNodeContainerPlain {
