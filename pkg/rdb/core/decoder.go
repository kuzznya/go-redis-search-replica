...
import (
	...
	"errors"
	"fmt"
	"github.com/kuzznya/go-redis-search-replica/pkg/rdb/memprofiler"
...
func (dec *Decoder) checkHeader() (int, error) {
	header := make([]byte, 9)
	...
	if err != nil {
		return 0, fmt.Errorf("io error: %v", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("%s is not valid version number", string(header[5:]))
	}
	if version < minVersion || version > maxVersion {
		return 0, fmt.Errorf("cannot parse version: %d", version)
	}
	return version, nil
}
...
func (dec *Decoder) parse(cb func(object model.RedisObject) bool, version int) error {
	var dbIndex int
	var expireMs int64
	for {
		b, err := dec.readByte()
...
func (dec *Decoder) Parse(cb func(object model.RedisObject) bool) (err error) {
	...
	version, err := dec.checkHeader()
	if err != nil {
		return err
	}
	return dec.parse(cb, version)
}
